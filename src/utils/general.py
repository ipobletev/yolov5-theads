...
def set_logging(name=None, verbose=VERBOSE):
    ...
    log.setLevel(level)
    log.addHandler(logging.FileHandler("../intemp/log/ova_" + str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')) + ".log"))
    handler = logging.StreamHandler()
    handler.setFormatter(logging.Formatter('[%(levelname)s] - %(asctime)s %(threadName)-10s: %(message)s'))
